...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
        ...
        let banned = jid.find( Jid => Jid === message.jid);
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '919656695625@s.whatsapp.net') {
...
Akku','Ayo kundan','Ayyappan','Bot','Chinnadan','Chunk','Chunke','FF','Friend','HBD','https','I am come','Kozhi','Kundan seta','Kundan','Lub','Mari','Minnu chechi','Miss u friend','mon','Njan vannu','Ok bye','Single','Song','thayoli','thayolli','Uyir','Rip','Seta','Sed','Sarasu','rip','praveen','Misab']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/' + a + '.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted: message.data, ptt: true})
